...
function PixelNode({ data }) {
  const accent = data.accent ?? "#ff1493";
  return (
    <div
      style={{
        background: "#0f0f0f",
    ...
    >
      ...
      {data.sub && (
        <div style={{ color: "#aaa", fontSize: 14, marginTop: 2 }}>{data.sub}</div>
      )}
...
const lbl = (t) => ({ label: t, labelStyle: { fill: "#aaa", fontFamily: "VT323", fontSize: 13 } });
...
function DiagramInner() {
  ...
  return (
    ...
    >
      ...
      >
        BACKLANDS — Technology Architecture
      </div>
      <div
        style={{
          position: "absolute",
